...
F1.load_state_dict(torch.load(os.path.join(args.checkpath, "F1_iter_model_{}_{}_to_{}_step_{}.pth.tar".format(args.method, args.source, args.target, args.step))))
    ...
    with open(output_file, "w") as f:
        with torch.no_grad():
            for batch_idx, data_t in tqdm(enumerate(loader)):
                ...
                pred1 = output1.max(1)[1]  # get the index of the max log-probability
